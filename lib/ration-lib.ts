 ...
 */
export function getVideoRatioFromSrc(src: string): Promise<number> {
  return new Promise((resolve) => {
    ...
    video.onerror = () => {
      resolve(16 / 9) // Default to common video aspect ratio on error
    }
  })
}
 ...
 */
export function getImageRatioFromSrc(src: string): Promise<number> {
  return new Promise((resolve) => {
    const img = new Image()
    img.src = src
    img.onload = () => {
    ...
    img.onerror = () => {
      resolve(4 / 3) // Default to common image aspect ratio on error
    }
  })
}
 ...
 */
export function getHeightFromWidth(width: number | string, ratio: number): number {
  const parsedWidth = typeof width === "string" ? Number.parseFloat(width) : width
  if (isNaN(parsedWidth) || ratio === 0) {
    return 0
  }
  return Math.round(parsedWidth / ratio)
}

/**
 * Calculates width based on height and aspect ratio
 */
export function getWidthFromHeight(height: number, ratio: number): number {
  if (height <= 0) {
...
const height = getHeightFromWidth(800, ratio1.ratio);
console.log(height); // 450 (800 ÷ 1.777... = 450)

// Calculate width from height and ratio
const width = getWidthFromHeight(400, ratio1.ratio);
console.log(width); // 711 (400 × 1.777... = 711)

// Calculate full dimensions
const dimensions1 = calculateDimensions({ width: 1200 }, ratio1.ratio);
console.log(dimensions1); // { width: 1200, height: 675 }

const dimensions2 = calculateDimensions({ height: 600 }, ratio1.ratio);
...
if (file) {
  getMediaRatioFromFile(file).then(ratio => {
    ...
    const mobileDimensions = calculateDimensions({ width: 320 }, ratio.ratio);
    console.log(`Mobile (320px): ${mobileDimensions.width}x${mobileDimensions.height}`);
    
    const desktopDimensions = calculateDimensions({ width: 1920 }, ratio.ratio);
