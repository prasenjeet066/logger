...
export async function GET(request: NextRequest, { params }: { params: { username: string } }) {
  try {
    ...
    const posts = await Post.find({
      ...
      isRepost: false, // Exclude reposts
      parentPostId: null, // Exclude replies
    })
      .sort({ createdAt: -1 })
      .limit(20) // Limit to 20 posts for now, can add pagination
      .lean()

    const formattedPosts = await formatPostsWithInteractionStatus(posts, currentUserId)
