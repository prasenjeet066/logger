...
export async function GET(request: NextRequest, { params }: { params: { username: string } }) {
  try {
    ...
    const mediaPosts = await Post.find({
      ...
      mediaUrls: { $exists: true, $ne: [] }, // Posts with media
    })
      .sort({ createdAt: -1 })
      .limit(20)
      .lean()

    const formattedMediaPosts = await formatPostsWithInteractionStatus(mediaPosts, currentUserId)

    return NextResponse.json(formattedMediaPosts)
  } catch (error) {
    console.error("Error fetching user media posts:", error)
    return NextResponse.json({ error: "Failed to fetch user media posts" }, { status: 500 })
  }
}
