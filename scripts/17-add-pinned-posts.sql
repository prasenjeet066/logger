...
CREATE INDEX IF NOT EXISTS idx_posts_pinned ON public.posts(user_id, is_pinned, created_at DESC);

-- Update the timeline function to prioritize pinned posts
...
-- Function to pin/unpin a post
CREATE OR REPLACE FUNCTION toggle_pin_post(post_uuid UUID, user_uuid UUID)
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
  ...
  SELECT is_pinned INTO current_pinned
  FROM public.posts
  WHERE id = post_uuid AND user_id = user_uuid;
  
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Post not found or user does not own this post';
  END IF;
  
  -- If trying to pin, check if user already has 3 pinned posts
  IF NOT current_pinned THEN
    SELECT COUNT(*) INTO pinned_count
    FROM public.posts
    WHERE user_id = user_uuid AND is_pinned = true;
    
    IF pinned_count >= 3 THEN
      RAISE EXCEPTION 'Maximum of 3 pinned posts allowed';
    END IF;
  END IF;
...
-- Function to get user's pinned posts
CREATE OR REPLACE FUNCTION get_pinned_posts(user_uuid UUID)
RETURNS TABLE (
  id UUID,
  content TEXT,
  created_at TIMESTAMP WITH TIME ZONE,
...
BEGIN
  ...
  WHERE p.user_id = user_uuid AND p.is_pinned = true
